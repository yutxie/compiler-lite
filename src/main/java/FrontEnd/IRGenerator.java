            ...
            default:
                ...
                switch (node.op) {
                    case LSHIFT: case RSHIFT:
                        if (!(node.rhs instanceof ConstantNode)) {
                            move = new Move();
                            move.dst = new Register("rcx");
                            move.src = node.rhs.value;
                            codeList.addLast(move);
                            bin.src = new Register("cl");
                        }
                        if (node.op == LSHIFT) bin.type = Binary.Type.SAL;
                        else bin.type = Binary.Type.SAR;
                        break;
                    case ADD: bin.type = Binary.Type.ADD; break;
